namespace PhotoBooth.Abstraction.Configuration
{
    public class ConfigurationKeys
    {
        public const string StepDownDurationInSeconds = "step_down_duration";
        public const string CaptureCountDownStepCount = "capture_count_down_steps";
        public const string ReviewCountDownStepCount = "review_count_down_steps";
        public const string ReviewImageWidth = "review_image_width_key";
        public const string ReviewImageQuality = "review_image_compression";
        public const string SelectedPrinter = "selected_printer_key";
        public const string SelectedCamera = "selected_camera_key";
    }
}
